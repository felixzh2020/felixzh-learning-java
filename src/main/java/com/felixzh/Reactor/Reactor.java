...
/**
 * com.felixzh.Reactor 接收连接，直接负责 I/O 的读写，拿到字节后有单线程和多线程两种处理器：
 * <p>
 * 单线程处理器：业务处理也由 com.felixzh.Reactor 线程来做
 * <br>
 * 多线程处理器：业务处理由线程池线程来做
 *
 * @author tongwu.net
    ...
    class Acceptor implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (sc != null) {
                    // 把提示发到界面
                    sc.write(ByteBuffer.wrap("Implementation of com.felixzh.Reactor Design Partten by tonwu.net\r\nreactor> ".getBytes()));
...
//					new MultithreadHandler(selector, sc); // 线程池处理连接
    ...
    public static void main(String[] args) {
        try {
            ...
            th.setName("com/felixzh/Reactor");
            th.start();
            th.join();
        } catch (Exception e) {
            e.printStackTrace();
