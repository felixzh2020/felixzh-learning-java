...
/**
 * 单线程基本处理器，I/O 的读写以及业务的处理均由 com.felixzh.Reactor 线程完成
 *
 * @author felixzh
 * @see Reactor
 */
public class BasicHandler implements Runnable {
    ...
    BasicHandler(Selector sel, SocketChannel sc) throws IOException {
        socket = sc;
     ...
     */
    boolean inputIsComplete(int bytes) throws IOException {
        if (bytes > 0) {
            input.flip(); // 切换成读取模式
     ...
     */
    boolean outputIsComplete(int written) {
        if (written <= 0) {
            // 用户只敲了个回车， 断开连接
            return true;
        }
